    ...
    <div className="flex justify-center mb-4 sm:mb-6 md:mb-8 overflow-x-auto py-4 sm:py-6 md:py-8 px-2 sm:px-4">
      <div className="flex items-center min-w-fit">
        {steps.map((step, index) => {
          ...
          if (isActive) {
            circleClasses += ` bg-gradient-to-br from-${step.color}-50 to-${step.color}-100 dark:from-${step.color}-900/20 dark:to-${step.color}-800/20 border-${step.color}-500 text-${step.color}-700 dark:text-${step.color}-300 shadow-lg`;
            labelClasses += ` text-${step.color}-700 dark:text-${step.color}-300`;
          } else if (isCompleted) {
            circleClasses += ` bg-gradient-to-br from-${step.color}-500 to-${step.color}-600 border-${step.color}-500 text-white shadow-md`;
            labelClasses += " text-gray-700 dark:text-gray-300";
          } else {
            circleClasses +=
              " bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 border-gray-300 dark:border-gray-600 text-gray-400 dark:text-gray-500";
            ...
            <div key={index} className="flex items-center">
              ...
              >
                ...
                <div
                  className={`absolute -top-1 sm:-top-2 -right-4 sm:-right-5 md:-right-6 w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center transition-all duration-300 ${
                    isActive
                      ? `bg-${step.color}-500 text-white shadow-lg ring-2 ring-white dark:ring-gray-800 scale-110`
                      : isCompleted
                      ? `bg-${step.color}-100 dark:bg-${step.color}-900/30 text-${step.color}-700 dark:text-${step.color}-300 shadow-md`
              ...
              {index < steps.length - 1 && (
                <div className="mx-2 sm:mx-3 md:mx-4 lg:mx-6 relative">
                  <div className="h-0.5 w-8 sm:w-10 md:w-12 lg:w-16 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ease-out ${
                        currentStep > stepNumber + 1
                          ? `bg-gradient-to-r from-${step.color}-500 to-${
                              steps[index + 1].color
                            }-500`
                  ...
                  {currentStep === stepNumber + 1 && (
                    <ChevronRight
                      className={`absolute top-1/2 left-[98%] transform -translate-x-1/2 -translate-y-1/2 w-3 h-3 sm:w-4 sm:h-4 text-${step.color}-500 animate-pulse`}
                    />
                  )}
                </div>
              )}
